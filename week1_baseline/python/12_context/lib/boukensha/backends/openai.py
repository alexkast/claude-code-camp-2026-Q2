...
class OpenAI(Base):
    ...
    def to_input(self, messages: list[Any]) -> list[dict[str, Any]]:
    ...
    def _assistant_items(self, content: Any) -> list[dict[str, Any]]:
        blocks = [{"type": "text", "text": content}] if isinstance(content, str) else content

        text = "".join(b["text"] for b in blocks if b["type"] == "text")
        items: list[dict[str, Any]] = [] if not text else [{"role": "assistant", "content": text}]
