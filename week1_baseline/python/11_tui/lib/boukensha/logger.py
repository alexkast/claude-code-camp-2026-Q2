...
class Logger:
    ...
    def _task_name(self, task: Any) -> str | None:
        if task is None:
            return None
        return task.task_name() if hasattr(task, "task_name") else str(task)

    def _provider_name(self, backend: Any) -> str | None:
        if backend is None:
            return None
    ...
    def _first_integer(self, data: dict[str, Any], *keys: str) -> int | None:
        for key in keys:
            value = data.get(key)
    ...
    def _estimate_cost(self, backend: Any, tokens: dict[str, int | None]) -> float | None:
